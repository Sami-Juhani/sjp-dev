...
class Media {
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      uniform bool uShouldReduceMotion;
    ...
    `,
      ...
      void main() {
        ...
        // Apply border radius
        vec2 st = vUv * 2.0 - 1.0;
        float d = length(max(abs(st) - (1.0 - uBorderRadius), 0.0));
        float a = 1.0 - smoothstep(0.0, uBorderRadius, d);
    ...
    `,
      uniforms: {
        ...
        uShouldReduceMotion: { value: this.shouldReduceMotion === true }
      },
      transparent: true
    })
    const img = new Image()
...
class App {
  ...
  onCheck() {
    ...
    if (this.onIndexChange) {
      ...
      setTimeout(() => {
        if (this.onIndexChange) this.onIndexChange(actualIndex)
      }, 200)
    }
  }
  ...
  addEventListeners() {
    ...
    // Only start drag from container
    this.container.addEventListener('mousedown', this.boundOnTouchDown)
    this.container.addEventListener('touchstart', this.boundOnTouchDown)
    // But track movement and release globally
